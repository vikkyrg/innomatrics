      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto grid max-w-2xl grid-cols-1 gap-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {blogPosts.map((post, index) => (
            <article 
              key={index} 
              className="flex flex-col items-start bg-white rounded-lg overflow-hidden group transition-all duration-300"
            >
              ...
              <div className="flex flex-col flex-1 p-6">
                <div className="flex-1">
                  <h3 className="text-xl font-semibold leading-7 text-primary-900 group-hover:text-primary-700 transition-colors">
                    <Link to="#">
                      <span className="absolute inset-0" aria-hidden="true" />
                      {post.title}
                    </Link>
                  </h3>
                  <p className="mt-3 text-base leading-7 text-secondary-700">
                    {post.excerpt}
                  </p>
                </div>
              </div>
